...
if not os.path.exists('../data/hpc_data_hub/Pandemic/casesAndDeaths.csv'):
    with ZipFile('../data/hpc_data_hub/Pandemic.zip', 'r') as zip_obj:
        zip_obj.extractall('../data/hpc_data_hub/')  # Extract all the contents in the same directory
...
df_list = [df.loc[df['percent_rural_population'] <= 5],
           df.loc[(df['percent_rural_population'] >= 45) & (df['percent_rural_population'] <= 55)],
           df.loc[df['percent_rural_population'] == 100]
           ]
county_rurality_list = ['mostly_urban', 'urban_rural', 'rural']

for df, county_rurality in zip(df_list, county_rurality_list):
